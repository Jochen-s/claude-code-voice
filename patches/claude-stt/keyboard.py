...
def _output_via_injection(
    ...
    try:
        ...
        # Clipboard + Ctrl+V paste to avoid pynput SendInput duplication.
        # pynput's kb.type() uses SendInput per-character which gets doubled
        # by the WH_KEYBOARD_LL hook on Windows. Clipboard paste is atomic.
        import platform as _plat
        if _plat.system() == "Windows":
            return _output_via_clipboard_paste(text, config)

        # Non-Windows: use pynput directly
        kb = get_keyboard()
        kb.type(text)
...
def _send_ctrl_v() -> None:
    """Send Ctrl+V via pynput Controller (4 keystrokes, not per-character)."""
    kb = get_keyboard()
    kb.press(Key.ctrl)
    kb.press('v')
    kb.release('v')
...
def _output_via_clipboard_paste(text: str, config: Config) -> bool:
    """Output text via clipboard + native Ctrl+V paste.

    Bypasses pynput's kb.type() which causes duplication on Windows due to
    WH_KEYBOARD_LL hook feedback with SendInput per-character injection.
    """
    try:
        import pyperclip
    except ImportError:
        _logger.warning("pyperclip not installed; falling back to kb.type()")
        kb = get_keyboard()
        kb.type(text)
        return True
