...
def _try_acquire_singleton_mutex():
    """Acquire a Windows named mutex to enforce single daemon instance.
    Returns mutex handle if acquired (first instance), None if already held."""
    if os.name != "nt":
        return None  # Unix uses different mechanism
    try:
        import ctypes
        kernel32 = ctypes.windll.kernel32
        handle = kernel32.CreateMutexW(None, True, "Global\\claude-stt-daemon-singleton")
        ERROR_ALREADY_EXISTS = 183
        if kernel32.GetLastError() == ERROR_ALREADY_EXISTS:
            if handle:
                kernel32.CloseHandle(handle)
            return None
        return handle  # Caller must keep alive for daemon lifetime
    except Exception:
        logger.warning("Failed to acquire singleton mutex", exc_info=True)
        return None  # Treated as "held" — safe-fail prevents duplicate daemons
...
def start_daemon(background: bool = False):
    ...
    # OS-level singleton guard (Windows) — prevents TOCTOU race in PID check.
    # Only acquired by the actual daemon process (foreground/run path), NOT by
    # the parent that calls _spawn_background(). The child acquires its own mutex.
    global _daemon_mutex_handle
    if os.name == "nt":
        mutex = _try_acquire_singleton_mutex()
        if mutex is None:
            logger.info("Daemon is already running (mutex held).")
            return
...
def stop_daemon():
    ...
    # Note: The daemon process's mutex is released automatically by Windows
    # when the process exits (via taskkill above). This block handles the
    # edge case where stop_daemon() is called in-process (e.g., signal handler).
    global _daemon_mutex_handle
    if _daemon_mutex_handle is not None:
        try:
            import ctypes
            ctypes.windll.kernel32.ReleaseMutex(_daemon_mutex_handle)
            ctypes.windll.kernel32.CloseHandle(_daemon_mutex_handle)
        except Exception:
            pass
        _daemon_mutex_handle = None
